...
function submitAction() {
  ...
  if (length > 0) {
    ...
    warningAlert("Please Enter Accessories Roll");
  }
}
...
function setAccessoriesReceiveInfo(transactionId) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#rollList").empty();
      drawAccessoriesRollListTable(accessoriesReceive.accessoriesRollList);
...
$("#noOfRoll").on('keyup', function (e) {
  if (e.key === 'Enter' || e.keyCode === 13) {
    ...
    const orderQty = $("#totalPoQty").text();
    const previousRecQty = $("#previousReceive").text();
    ...
    const accessoriesId = $("#accessoriesId").val();
    const accessoriesName = $("#accessoriesItem").text();
    const accessoriesColorId = $("#accessoriesColorId").val();
    const accessoriesColor = $("#accessoriesColor").text();
    const accessoriesRate = $("#accessoriesRate").val();
    ...
    if (noOfRoll != '') {
      if (receiveQty != '') {
        if (accessoriesId != '' && accessoriesName != '') {
          ...
          let rows = `<tr class='odd parentRowGroup-${length}'>
                    <td id='accessoriesName-${length}'>${accessoriesName}</td>
                    <td id='accessoriesColor-${length}'>${accessoriesColor}</td>
                    <td>${unit}</td>
                    <td>${orderQty}</td>
                    <td id='previousReceiveQty-${length}'>${previousRecQty}</td>
                    <td id='accessoriesReceiveQty-${length}'>0</td>
          ...
          for (let i = 1; i <= noOfRoll; i++) {
            ...
            rows += "<tr id='rowId-" + id + "' class='newRollRow rollRowList rowGroup-" + length + "' data-parent-row='" + length + "' data-purchase-order='" + purchaseOrder + "' data-style-id='" + styleId + "' data-item-id='" + itemId + "' data-item-color-id='" + itemColorId + "' data-accessories-id='" + accessoriesId + "' data-accessories-color-id='" + accessoriesColorId + "' data-roll-id='" + i + "' data-unit-id='" + unitId + "'>"
              + "<td id='rollId-" + id + "' contenteditable = 'true'>" + i + "</td>"
              + "<td>" + unit + "</td>"
              + "<td><input id='unitQty-" + id + "' type='number' onblur='totalRollQtyCount(" + length + ")' class='rollReceiveGroup-" + length + " form-control-sm max-width-100' value='" + qty + "'></td>"
              + "<td>" + rackSelect + "</td>"
              + "<td>" + binSelect + "</td>"
          ...
          $("#accessoriesReceiveQty-" + length).text(tempTotalReceive);
        } else {
...
function drawAccessoriesRollListTable(data) {
  const rackOptions = fakeRackList.map(rack => `<option value=${rack.rackId}>${rack.rackName}</option>`);
