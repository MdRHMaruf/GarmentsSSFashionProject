...
<%
    ...
    try {
		...
		String Sql="select *,(select Name from tbBuyer where id=a.BuyerId) as BuyerName,(select StyleNo from TbStyleCreate where StyleId=a.StyleId) as StyleNo,(select ItemName from tbItemDescription where ItemId=a.ItemId) as ItemName,(select LineName from TbLineCreate where LineId=a.LineId) as LineName,'"+layoutName+"' as layoutName,(select Name from TbEmployeeInfo where AutoId=a.EmployeeId) as EmployeeName,(select TypeName from TbNarration where Category='"+layoutCategory+"' and Type=a.Type) as TypeName from tbLayoutPlanDetails a where a.BuyerId='"+buyerId+"' and a.BuyerOrderId='"+buyerorderId+"' and a.StyleId='"+styleId+"' and a.ItemId='"+itemId+"' and a.date='"+layoutDate+"' and a.Type in("+layoutType+")";
      	System.out.println("sql "+Sql);
      	
		String jrxmlFile = session.getServletContext().getRealPath("WEB-INF/jasper/production/ProductionRejectDetailsReport.jrxml");
